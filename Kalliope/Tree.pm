...
sub _doNode {
	...
	if ($this_HasChildren) {
	    if ($DOM_HTML) {
		$thisGfx = qq|<IMG onclick="TreeNodeExpand($nodeId)" onfocus="blur()" ID="treeNodeGfx$nodeId" NODETYPE="L" SRC="$gfxdir/TreeEndPlus.gif">|;
	    } else {
		$thisGfx = qq|<IMG SRC="$gfxdir/TreeEndMinus.gif">|;
	    }
	} else {
	    $thisGfx = qq|<IMG SRC="$gfxdir/TreeEnd.gif">|;
	}
	$subGfx  = qq|<IMG SRC="$gfxdir/TreeNone.gif">|;
    } else {
	if ($this_HasChildren) {
	    if ($DOM_HTML) {
		$thisGfx = qq|<IMG onclick="TreeNodeExpand($nodeId)" onfocus="blur()" ID="treeNodeGfx$nodeId" NODETYPE="T" SRC="$gfxdir/TreeTPlus.gif">|;
	    } else {
		$thisGfx = qq|<IMG SRC="$gfxdir/TreeTMinus.gif">|;
	    }
	} else {
	    $thisGfx = qq|<IMG SRC="$gfxdir/TreeT.gif">|;
	}
	$subGfx  = qq|<IMG SRC="$gfxdir/TreeLine.gif">|;
    }

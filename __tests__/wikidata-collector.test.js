...
describe('Wikidata collector', () => {
  it('groups raw bindings and keeps raw and normalized values separate', () => {
    const observations = parseWikidataSnapshot({ results: { bindings: [
      ...
      { person: { value: 'http://www.wikidata.org/entity/Q1' }, personLabel: { value: 'Test, Person' }, viaf: { value: '123' } },
    ] } }, provenance);
    expect(observations).toHaveLength(1);
    expect(observations[0]).toMatchObject({ sourceId: 'Q1', original: { aliases: ['T. Person'], rawBindings: expect.any(Array) }, normalized: { normalizedName: 'test person', birthYear: '1900', identifiers: { wikidata: 'Q1', viaf: '123' } }, provenance: { snapshotSha256: 'snapshot' } });
  });

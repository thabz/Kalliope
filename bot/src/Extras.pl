...
sub Extras {
    ...
    if ($message =~ /^person/i) {
	if (my $resp = KalliopePerson::get($message)) {
	    return $resp;
	}
    }
    if ($message =~ /^værker/i) {
	if (my $resp = KalliopeWorks::get($message)) {
	    return $resp;
	}
    }
    if ($message =~ /^år/i) {
	if (my $resp = KalliopeYear::get($message)) {
	    return $resp;
	}
    }
    if ($message =~ /^primær/i) {
	if (my $resp = KalliopePrimaer::get($message)) {
	    return $resp;
	}
    }
    if ($message =~ /^digt/i) {
    ...
    if ($message =~ /^dagen/i) {
	if (my $resp = KalliopeDag::get($message)) {
	    return $resp;
	}
    }
	...
	if ($message =~ /^\s*(?:ex)?change\s+([\d\.\,]+)\s+(\S+)\s+(?:into|to|for)\s+(\S+)/) {

...
import { force_reload as globalForceReload } from '../libs/caching.js';
...
const buildIncrementalSql = (collected, changedFiles) => {
  ...
  if (workIds.size > 0) {
    ...
    statements.push(
      `DELETE FROM text_search_index_fts WHERE rowid IN (SELECT rowid FROM text_search_index WHERE text_id IN ${textIdSubquery});`
    );
  ...
  if (textIdsForEventsAndFts.length > 0) {
    ...
    statements.push(
      `INSERT INTO text_search_index_fts (rowid, raw_text) SELECT rowid, raw_text FROM text_search_index WHERE text_id IN (${toSqlInList(textIdsForEventsAndFts)});`
    );
  }
...
export const updateSqliteIndex = (collected) => {
  ...
  if (!globalForceReload && canSkipImport && fs.existsSync(SQLITE_PATH)) {
    return true;
  }
